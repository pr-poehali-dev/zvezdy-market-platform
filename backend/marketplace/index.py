        ...
        if method == 'GET':
                ...
                if action == 'list':
                    cur.execute('''
                        SELECT ug.id as user_gift_id, ug.sale_price, ug.purchased_at,
                               ...
                               (SELECT COUNT(*) FROM gift_transactions WHERE user_gift_id = ug.id) as transaction_count
                        FROM user_gifts ug
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                if action == 'buy_from_store':
                    ...
                    cur.execute('SELECT * FROM gifts WHERE id = %s AND is_active = TRUE', (gift_id,))
                    gift = cur.fetchone()
                    ...
                    cur.execute('UPDATE users SET balance = balance - %s WHERE id = %s', (gift['price'], user_id))
                    
                    cur.execute('''
                        INSERT INTO user_gifts (user_id, gift_id, purchase_price)
                        VALUES (%s, %s, %s)
                        RETURNING id
                    ''', (user_id, gift_id, gift['price']))
                    user_gift = cur.fetchone()
                    
                    cur.execute('''
                        INSERT INTO gift_transactions (gift_id, user_gift_id, buyer_id, price, transaction_type)
                        VALUES (%s, %s, %s, %s, 'store_purchase')
                    ''', (gift_id, user_gift['id'], user_id, gift['price']))
                    
                    cur.execute('''
                        INSERT INTO balance_transactions (user_id, amount, transaction_type, description)
                        VALUES (%s, %s, 'gift_purchase', %s)
                    ''', (user_id, -gift['price'], f"Покупка подарка: {gift['name']}"))
                    
                    if gift['is_limited']:
                        cur.execute('''
                            UPDATE gifts SET remaining_supply = remaining_supply - 1
                            WHERE id = %s AND remaining_supply > 0
                        ''', (gift_id,))
                    ...
                    cur.execute('''
                        UPDATE user_gifts SET user_id = %s, is_on_sale = FALSE, sale_price = NULL
                        WHERE id = %s
